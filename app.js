...
async function loadProfile() {
  ...
  if (profTickEl) {
    ...
    profTickEl.innerHTML = ownVerif ? '<svg class="vtick" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><circle cx="12" cy="12" r="10" fill="#8B5E3C"/><path d="M8 12l2.5 2.5L16 9" stroke="white" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"/></svg>' : '';
...
async function openArchivistDrawer(userId) {
  ...
  if (arcNameEl) {
    const arcTick = (p.verified || p.is_verified || p.isAdmin || p.is_admin) ? '<svg class="vtick vtick-arc" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><circle cx="12" cy="12" r="10" fill="#8B5E3C"/><path d="M8 12l2.5 2.5L16 9" stroke="white" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"/></svg>' : '';
...
let STARRED_IDS    = new Set(); // loaded per-user in loadProfile()
...
function saveStarred() {
  if (!CUR_USER) return;
  localStorage.setItem('croniq_starred_' + CUR_USER.id, JSON.stringify([...STARRED_IDS]));
}
...
function userCardHTML(u, miniStar = false) {
  ...
  const av = avSrc
    ...
    : ini(u.name);
  const tick = (isVerif || isAdmU) ? `<svg class="vtick vtick-sm" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><circle cx="12" cy="12" r="10" fill="#8B5E3C"/><path d="M8 12l2.5 2.5L16 9" stroke="white" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"/></svg>` : '';
  ...
  return `<div class="user-card${isAdmU ? ' uc-is-admin' : ''}" onclick="openArchivistDrawer('${u.id}')">
    ...
    <div class="uc-info">
      <div class="uc-name-row">
        <span class="uc-name">${escapeHtml(u.name)}</span>${tick}
      </div>
      <div class="uc-handle">@${escapeHtml(u.handle)}</div>
      ${u.bio ? `<div class="uc-bio">${escapeHtml(u.bio).slice(0,40)}${u.bio.length>40?'...':''}</div>` : ''}
      <div class="uc-tags">
...
══════════════════════════════════════════════════════════ */
const VTICK_16 = '<svg class="vtick" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><circle cx="12" cy="12" r="10" fill="#8B5E3C"/><path d="M8 12l2.5 2.5L16 9" stroke="white" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"/></svg>';
const VTICK_13 = '<svg class="vtick vtick-sm" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><circle cx="12" cy="12" r="10" fill="#8B5E3C"/><path d="M8 12l2.5 2.5L16 9" stroke="white" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"/></svg>';
